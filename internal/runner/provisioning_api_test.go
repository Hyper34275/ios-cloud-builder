...
func TestDownloadASCProvisioningProfilesUsesExactBundle(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/v1/bundleIds":
			if r.URL.Query().Get("filter[identifier]") != "com.example.app" {
	...
	defer server.Close()
	client := testASCClient(t, server.Client())
...
func TestFindASCBundleIDRegistersMissingIdentifier(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/v1/bundleIds" && r.Method == http.MethodPost:
			...
			writeASCJSON(w, `{"data":{"type":"bundleIds","id":"bundle-new","attributes":{"identifier":"com.example.new","platform":"IOS"}}}`)
		default:
			t.Fatalf("unexpected request: %s %s", r.Method, r.URL.Path)
		}
	}))
	...
	client := testASCClient(t, server.Client())
	client.baseURL = server.URL
	resourceID, err := findASCBundleID(context.Background(), client, "com.example.new")
	if err != nil {
		t.Fatal(err)
	}
	if resourceID != "bundle-new" {
		t.Fatalf("resourceID = %q", resourceID)
	}
	encoded, _ := json.Marshal(createBody)
	if body := string(encoded); !strings.Contains(body, `"identifier":"com.example.new"`) || !strings.Contains(body, `"platform":"IOS"`) {
		t.Fatalf("create body %s missing expected fields", body)
	}
}

func TestFindASCBundleIDRejectsAmbiguousMatch(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		writeASCJSON(w, `{"data":[`+
			`{"type":"bundleIds","id":"bundle1","attributes":{"identifier":"com.example.app","platform":"IOS"}},`+
			`{"type":"bundleIds","id":"bundle2","attributes":{"identifier":"com.example.app","platform":"UNIVERSAL"}}]}`)
	}))
	defer server.Close()
	client := testASCClient(t, server.Client())
	client.baseURL = server.URL
	if _, err := findASCBundleID(context.Background(), client, "com.example.app"); err == nil {
		t.Fatal("ambiguous bundle identifier match was accepted")
	}
}
...
func TestFindASCCertificateRejectsNonDistributionType(t *testing.T) {
	...
	if _, err := findASCCertificate(context.Background(), client, hex.EncodeToString(fingerprint[:])); err == nil {
		t.Fatal("development certificate was accepted for a distribution signature")
	}
}

func TestFindASCCertificateRejectsExpiredCertificate(t *testing.T) {
	certificate := []byte("expired certificate")
	fingerprint := sha1.Sum(certificate) // #nosec G401 -- mirrors Apple's signing identity fingerprint.
	attributes, _ := json.Marshal(map[string]any{
		"certificateType": "DISTRIBUTION", "certificateContent": base64.StdEncoding.EncodeToString(certificate),
		"expirationDate": time.Now().Add(-24 * time.Hour).UTC(),
